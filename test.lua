...
local function LoadConfiguration(Configuration)
    -- Decode the JSON data from the Configuration string
    local Data = HttpService:JSONDecode(Configuration)

    -- Iterate through each key-value pair in the decoded data
    for FlagName, FlagValue in pairs(Data) do
        -- Check if the flag exists in ArrayFieldLibrary.Flags
        local flag = ArrayFieldLibrary.Flags[FlagName]
        if flag then
            -- Run the setting of the flag in a new coroutine
            spawn(function()
                -- Determine the type of the flag
                if flag.Type == "ColorPicker" then
                    -- If it's a ColorPicker type, set the unpacked color
                    flag:Set(UnpackColor(FlagValue))
                else
                    -- Check if the flag's current value differs from the loaded value
                    local currentValue = flag.CurrentValue or flag.CurrentKeybind or flag.CurrentOption or flag.Color
                    if currentValue ~= FlagValue then
                        -- If the values differ, set the flag to the loaded value
                        flag:Set(FlagValue)
                    end
                end
            end)
        else:
            -- If the flag is not found, notify the user of the error
            ArrayFieldLibrary:Notify({
                Title = "Flag Error",
                Content = "ArrayField was unable to find '" .. FlagName .. "' in the current script."
            })
        end
    end
end
...
local function SaveConfiguration()
    -- Check if CEnabled is true before proceeding
    if not CEnabled then return end

    -- Initialize an empty table to store data
    local Data = {}

    -- Iterate through each key-value pair in ArrayFieldLibrary.Flags
    for key, flag in pairs(ArrayFieldLibrary.Flags) do
        -- Check the type of the flag
        if flag.Type == "ColorPicker" then
            -- If the type is "ColorPicker", pack the color value
            Data[key] = PackColor(flag.Color)
        else
            -- Otherwise, store the appropriate value based on the flag's properties
            Data[key] = flag.CurrentValue or flag.CurrentKeybind or flag.Color or flag.CurrentOption
        end
    end
    ...
    -- Define the file path using the configuration folder and file name
